...
$queue = get_option( YBS_CERT_QUEUE_OPT, [] );
if ( ! is_array( $queue ) ) $queue = [];
$state = ybs_cert_sender_get_state();
$log   = get_option( YBS_CERT_LOG_OPT, [] );
if ( ! is_array( $log ) ) $log = [];
...
?>
<div class="wrap">
	<h1 class="wp-heading-inline">Sertifika Gönder</h1>
	...
	<p>Kaynak liste: <code>assets/img/Katilimcilar/mailatilacaklar.csv</code><br>
	Sertifika yolu: <code>/assets/img/Katilimcilar/isim.jpg</code> (ek dosya bulunursa mail ekinde gönderilir, bulunmazsa mail içinde görsel/link verilir).</p>

	<div style="display:flex; gap:8px; flex-wrap:wrap; margin:12px 0 16px;">
		<form method="post"><?php wp_nonce_field( 'ybs_cert_sender_action' ); ?><input type="hidden" name="ybs_cert_action" value="prepare"><button class="button button-primary">CSV'den Kuyruk Oluştur</button></form>
		<form method="post"><?php wp_nonce_field( 'ybs_cert_sender_action' ); ?><input type="hidden" name="ybs_cert_action" value="start"><button class="button" style="background:#16a34a;color:#fff;border-color:#15803d;">Gönderimi Başlat</button></form>
		<form method="post"><?php wp_nonce_field( 'ybs_cert_sender_action' ); ?><input type="hidden" name="ybs_cert_action" value="stop"><button class="button" style="background:#dc2626;color:#fff;border-color:#b91c1c;">Durdur</button></form>
		<form method="post"><?php wp_nonce_field( 'ybs_cert_sender_action' ); ?><input type="hidden" name="ybs_cert_action" value="run_now"><button class="button">Şimdi 5 Adet Gönder</button></form>
		<form method="post"><?php wp_nonce_field( 'ybs_cert_sender_action' ); ?><input type="hidden" name="ybs_cert_action" value="send_test_enes"><button class="button" style="background:#2563eb;color:#fff;border-color:#1d4ed8;">ENES CANTÜRK Test Mail</button></form>
		<form method="post"><?php wp_nonce_field( 'ybs_cert_sender_action' ); ?><input type="hidden" name="ybs_cert_action" value="requeue_sent_all"><button class="button" style="background:#7c3aed;color:#fff;border-color:#6d28d9;">Gönderilenleri Tekrar Kuyruğa Al</button></form>
	</div>
	...
	<div style="background:#fff;border:1px solid #dcdcde;border-radius:6px;padding:12px;max-width:820px;">
		...
		<strong>Son 1 saatte deneme:</strong> <?php echo (int) $hour_count; ?> / 150
		<p style="margin:8px 0 0;color:#6b7280;">Tarayıcı kapansa da WordPress cron tetiklendikçe gönderim devam eder (siteye trafik geldikçe çalışır).</p>
	</div>
